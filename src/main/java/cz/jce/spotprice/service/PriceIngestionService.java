...
public class PriceIngestionService {
    ...
    private SyncResultDto performSync(boolean force) {
        ...
        // Delegating to PricePersistenceService so that @Transactional is applied
        // through Spring's proxy (self-calls on 'this' bypass the proxy).
        int count = pricePersistenceService.persist(awattarResponse.data(), eurToCzk, hash);
